...
**Before anything else: read `context/build-plan.md`'s "START HERE — Session Handoff" section, right after the Core Principle at the top of that file.** It's the real map for this project now — mission statement, current state, the full JobRight competitive teardown and differentiation strategy, and a pointer into the Master Feature Inventory (sections A–L) that tracks every feature ever discussed. This file (`RESUME.md`) stays as the fast technical-state summary; that section is the fast *product/strategy* summary. Read both.

**This session (2026-07-21) shipped Phase 0 — minimum-cost public launch controls** — RLS audit across every user-data table, per-user daily usage metering (`lib/usage.ts`), a Gemini-only cost policy for non-admin users (`lib/access.ts`), global feature kill switches (`lib/features.ts`), rate limiting on every AI route (`lib/rateLimit.ts`), a signup cap + `/waitlist` page, and — the real bug of the session — an `on_auth_user_created` trigger that auto-provisions a `profiles` row for every new OAuth signup. **Root cause found via live multi-account testing:** a second real Google account had no `profiles` row at all (no insert/trigger existed anywhere in the app), so every save silently failed with zero rows matched. Fixed live, backfilled null array/jsonb columns, verified with a real second account.

**Also this session:** a deep, systematic teardown of **JobRight** (the closest live competitor) — auth flow, onboarding, job list/detail pages, résumé editor workspace, interview question bank, settings, notifications — captured via the Claude-in-Chrome extension against a real logged-in session (two full recordings + live browsing). Every finding is in `build-plan.md`'s Master Feature Inventory and the new `§H1` job-detail-page anatomy table. Five **design-preview pages** were built from this research (`/preview`, `/preview/onboarding`, `/preview/resume`, `/preview/more`) — real components with dummy props, in Sortie's own tokens, verified live in both themes — covering résumé gap analysis, network signals, onboarding, the résumé editor workspace, and interview bank/settings/notifications. **Everything is committed and pushed to `origin/main`** — run `git status`/`git log` to confirm before assuming otherwise.

**Not yet done — the real gate before any public launch:** Phase 0 tasks #33–35 (Privacy Policy/Terms + account deletion, a real two-account isolation test, actual Vercel deployment). Do these before building anything net-new.

---

## Prior session (2026-07-20) — kept for continuity
