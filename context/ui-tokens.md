...
**⚠️ Real contrast gap, flagged 2026-08-20 (accessibility audit), not yet fixed**: `--color-text-muted` (#8A8F89) computes to only ~3.3:1 against white surfaces — below WCAG AA's 4.5:1 minimum for normal text (1.4.3), verified by hand via the real relative-luminance formula. This token is used for genuinely legible informational text (timestamps, secondary labels) across the app, not decorative/exempt content. A verified fix candidate exists (~`#6B706B` lands at ~5.05:1) but needs a real visual look-and-feel check before landing — this is a deliberate, hand-tuned "quiet" tone, not an oversight, so don't silently swap the hex without a real before/after look.
...
Confirmed **four separate times** now — a 4th hit 2026-08-18 building `.dashboard-hero-card`/`.dashboard-well` (`app/globals.css`, dashboard visual-hierarchy pass): both classes' `box-shadow: inset ...` declarations (each sharing a rule with a `border`/`background` property) came back `"none"` in `getComputedStyle()`, confirmed live, not assumed. Fixed the same documented way — every `color-mix()`-bearing property moved into its own single-property rule. The three from before this system was fully documented: — this will recur on any future glass/gradient work in this file if not read first. Tailwind v4's build (Lightning CSS) auto-generates an `@supports (color: color-mix(in lab, red, red))` fallback block for any declaration using `color-mix()`, splitting the original rule into multiple fragments (a plain-color fallback version + the real `color-mix()` version gated behind `@supports`). This rewrite has, more than once, silently dropped *other, unrelated* properties that shared the same original declaration block:
