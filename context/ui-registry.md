...
Last updated: 2026-09-17 (Phase 58 — "Auto-fill with AI" rebuilt to fill visible fields, not skip them; Phase 56 templates/typography below).
...
**`FrameworkBar`** — the opt-in writing frameworks, rendered above the Action Plan on AI Rewrite and again at the top of the Editor tab (same picker also reused standalone as `FrameworkPicker`, via `FrameworkRewritePanel`, inside the Editor tab's per-bullet menu). Three steps: framework → bullet → that framework's questions. Each framework shows its expansion and its **documented weakness at choosing time**, so a framework that is wrong for someone's level (SOAR on a junior role) can be avoided before committing. Step 2 repeats the formula and states that the listed bullets are the INPUT — a user seeing their existing bullets under a "Google XYZ" heading reasonably asked "how is this XYZ?". Step 3 is one labelled input per letter, badged with the slot it fills, carrying a real example as its placeholder; blank means that element is omitted, never guessed. **STAR pre-selected (Phase 57, direct user request):** both surfaces default their framework state to STAR (`getFramework("star")`) instead of `null`, so opening the picker jumps straight to step 2 — "All frameworks" still reaches the other four. **"Auto-fill with AI"** — a small teal `border-agent/40 bg-agent-muted` button next to the "answer what you can" line in step 3, with its own `Loader2`-spinner "Filling in…" state — calls `inferFrameworkAnswers()` (`actions/profile.ts`), a **separate, fast, single-bullet action** that fills the visible per-letter inputs in place (`setAnswers`), never touching `onApply`. The user reviews/edits the AI's inferred text exactly like manually-typed answers, then clicks the existing "Apply `<Framework>`" button themselves — same cost (one metered `bullet_rewrite`), same no-fabrication rule (a slot with no support in the bullet comes back empty). **Rebuilt in Phase 58** — the original Phase 57 design (`buildAutoFrameworkPrompt()`, now deleted) skipped the fields entirely and sent straight to the whole-résumé chat pipeline with the panel resetting immediately; direct live user report ("not filling anything... flickers... pathetic — no user ui showing") is why this shape exists instead.
...
Last updated: 2026-09-17 (Phase 58 — full width site-wide, see below; previously Phase 8, later same day — "Profile" moved off the top-level nav array onto the `UserCircle` icon, user-requested; previously rebuilt 2026-07-27 as a true 3-column grid to center the nav links, then made a floating inset rounded bar rather than edge-to-edge — matching Raycast's own nav structure, see Pattern notes)

**Phase 58 — full width, site-wide.** The inner bar's `max-w-[1400px]` cap (all 4 occurrences: authenticated/logged-out × desktop/mobile) was removed in favor of `w-full` — the floating-inset-bar treatment (rounded corners, `mx-4`/`sm:mx-6`/`lg:mx-8` side margins) stays exactly as-is, only the content-width cap is gone. Paired with the same change on every authenticated app-surface `<main>` (Résumé, Profile, Interview, Career, Settings, Notifications, Search, Compare, Cover Letter, News — each page's own `max-w-*` class removed, `w-full` kept). **Jobs pages are the deliberate exception**: Recommended/Search/Saved/External and their sub-pages kept their original `max-w-*` caps on direct user correction — those pages show a full-width Navbar over intentionally-narrower centered content, not a bug. Public/logged-out pages (using bare `<Navbar />`, not `<Navbar isAuthenticated>`) were left alone entirely — that authenticated-vs-public prop split is what decided scope.
...
File: components/profile/ResumeManager.tsx, components/profile/MigrateBaseResumeCard.tsx
Last updated: 2026-09-15 (Phase 57 — legacy base résumé folded into slots)
...
**`MigrateBaseResumeCard` (Phase 57):** `/resume` used to render a legacy single-résumé upload (`profiles.resume_pdf_url`, predating this slot system) as a separate, action-less "Base résumé" download link outside `ResumeManager` entirely — no primary/sync/edit. Now, when `hasUnmigratedBaseResume()` is true, a dashed `rounded-2xl border-dashed` prompt card renders above the table with a single "Add to résumé slots" button, calling `migrateBaseResumeToSlot()` and `router.refresh()` on success. **Must stay an explicit click, not an automatic side effect** — a first version ran the same migration inside `listResumes()` on every page read and raced itself into 96 duplicate rows for one account (concurrent requests all passed the check-then-insert before any single insert committed). `migrations/20260915190000_resumes-unique-storage-path.sql` (`UNIQUE (user_id, storage_path)`) is a DB-level backstop kept regardless of the click-only fix.
