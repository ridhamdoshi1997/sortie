...
| Ghost-job & layoff-risk company signals | 🔀 rescoped, shipped as Reappearance Signal | **Marker corrected 2026-08-19 (Phase 16)**: researched before an earlier session built anything — real department-level turnover/layoff data has no free, legal source (SEC filings only cover named executives, Glassdoor/Indeed scraping violates ToS, WARN Act only covers mass layoffs, confirmed via `lib/churnSignal.ts`'s own header comment). The honestly-buildable alternative — whether the same role at the same company keeps reappearing as a new listing across a user's own search history — is what actually shipped (`lib/churnSignal.ts`'s `ReappearanceSignal`). Ghost-listing detection at the per-posting level (vague comp, generic wording) is separately covered by the AI Navigator's legitimacy-signal read on the job detail page — not a company-level layoff-risk signal, which stays unbuilt for the reason above |
| **"Add job" — evaluate any external posting** (paste a job from anywhere, not just scanned sources) | ✅ | **Marker corrected 2026-08-19 (Phase 16)**: confirmed already shipped — `app/jobs/external`, `ExternalJobForm.tsx`, `lib/externalJob.ts`'s `createExternalJob` |
| **Saved-job liveness status** (Active / Closed sub-tabs on saved jobs) | ✅ shipped 2026-08-19 (Phase 16) | `components/shared/SavedJobsTabs.tsx`, `/saved-jobs` — splits on the existing `getListingSignal()` from Phase 10 |
...
| **Conversational per-job AI chat** — evidence-cited breakdown, free-text follow-up | ✅ absorbed into Navigator | **Marker corrected 2026-08-19 (Phase 16)**: a standalone `lib/jobChat.ts` was built for this, then found to genuinely duplicate Navigator (`lib/agentAssistant.ts`, shipped Phase 14) — Navigator's system prompt already lists "match-score diagnosis grounded in a job's own real evaluation" as capability #1, and auto-detects the focused job from the URL. The standalone chat was deleted; this need is covered by opening Navigator on any job-detail page |
...
| Application email drafts | ✅ | Phase 11 / F31 — `lib/emailDrafts.ts`/`actions/emailDrafts.ts`, cold/follow-up/thank-you types, real "Email Drafts" panel on job-detail |
| Resume version manager (slots, primary flag, per-résumé status) | ✅ | **Marker corrected 2026-08-30**: `is_primary` is a real column with a race-safe partial-unique-index-backed `setPrimaryResume()` (`actions/resumes.ts`), wired to `ResumeManager.tsx`'s real "N of 5 résumé slots used" UI — confirmed live, not design-only |
| **Per-job AI-generation version history** (regenerating a résumé/cover letter for the same job used to silently destroy the previous draft) | ✅ | 2026-08-28 (Phase 28) — `document_versions` table + `DocumentVersionHistory.tsx`. **Marker corrected 2026-08-30**: the blocking storage-download bug (`.download()`/CloudFront `ECONNRESET`) was fixed the same phase (`instrumentation.ts`'s `dns.setDefaultResultOrder("ipv4first")`) but never re-verified end-to-end afterward — now confirmed live: a real "View" download succeeded (real file returned, no error) and a real `restoreDocumentVersion()` call round-tripped successfully (200, ~2.7s, confirmed via server logs) |
| Follow-up & thank-you generator + timing nudges | ✅ | Both built — email draft types cover follow-up/thank-you (`lib/emailDrafts.ts`), timing nudges are `lib/followUpNudge.ts` + `FollowUpNudge.tsx` on job-detail |
| DOCX export | ✅ | `app/api/documents/download-docx/route.ts`, real "DOCX" button on job-detail, confirmed live |
| Markdown export | ❌ unbuilt | No route/UI found for it — DOCX shipped, Markdown didn't |
| **Resume fit / gap analysis** (score 0–10, per-check gaps, keyword coverage) | ✅ | **Marker corrected 2026-08-30**: fully live (`lib/resumeQuality.ts`/`ATSAuditCard.tsx`/`QualityGradeCard.tsx`), confirmed via a real fit-score card (8.5/10, "Strong fit") during this session's own testing — not design-preview-only |
...
| LinkedIn OAuth | ✅ | **Marker corrected 2026-08-27 (Phase 28)**: confirmed already shipped and live — `components/auth/LoginCard.tsx`'s "Continue with LinkedIn" button posts to `app/api/auth/oauth/[provider]/route.ts` (generic InsForge `signInWithOAuth`, same code path as the already-✅ Google/GitHub rows, also handles Apple/Microsoft). Live-verified: navigating the route redirects to a real LinkedIn authorization page (client ID accepted) — stopped there, no credentials entered. Session had mis-attributed this to "Inngest" (background-job runner) instead of **InsForge** (the auth/backend provider) — easy mix-up, same as the earlier stale-marker sweep. Note: LinkedIn's API only returns name/email/photo, **not** work history, so résumé upload stays the real import path; see the separate "LinkedIn profile import (onboarding)" row below for that distinct, still-open feature |
...
| Feature | Status | Source |
| --- | --- | --- |
| Split contact fields (name parts, phone type/country code, structured address) | ✅ shipped 2026-08-28 (Phase 28) | Additive columns on `profiles`, `ProfileForm.tsx`'s "Name & address breakdown" block. `full_name`/`phone`/`location` untouched — every existing consumer still reads those |
| **Equal Employment / voluntary self-ID** (race/ethnicity, gender, veteran status, disability — the standard EEOC fields many ATS applications request) | ✅ shipped 2026-08-28 (Phase 28) | `saveEeocInfo()`/`clearEeocInfo()`, its own consent-gated `SectionModal` in `ProfileForm.tsx` — server-checked consent (`eeoc_consented_at`), never bundled into the general profile save, live-verified withdraw-consent round trip |
...
| **STAR Story Vault** (new idea from this research pass) | ✅ | **Marker corrected 2026-08-19 (Phase 16)**: confirmed already shipped — `star_stories` table (situation/task/action/result columns), `actions/starStories.ts`, `components/career/StarVault.tsx`. Distinct from §N's STAR Story Matrix (that one maps stories to *predicted interview questions*; this one is about capturing the story in the first place — both shipped |
| **"Why I Left" private log** (new idea from this research pass) | ✅ shipped 2026-08-19 (Phase 16) | `jobs.reflection_loved`/`jobs.reflection_avoid`, `actions/jobs.ts`'s `updateJobReflection`, `components/job-details/WhyILeftReflection.tsx` — shown only when a job is marked rejected. Zero external data, exactly as scoped here |
...
| 3. Live Fire | **Live-Fire Scenario Simulator** | ⏸ deferred to post-launch (2026-08-27, Phase 28, direct user decision — "maybe," not committed) | Concrete version of the old "live mock-interview simulator" row — conversational, roleplays as the actual named interviewer from Panel Topology, questions rooted in the real Moat Briefing content, not generic prompts. Real-time VOICE feasibility researched this session, see "Live-Fire Simulator — voice architecture research" below. Do not pick this up unprompted |
| — | **Inline code-editor "Practice Sandbox"** | ✅ shipped | **Marker corrected 2026-08-30**: `components/interview/PracticeSandbox.tsx` is real and wired into `QuestionBankPanel.tsx` — Phase 26's interview-page restructure ("Live Code Editor" capability strip, "Code editor" badge on technical question rows) built this; the marker here was just never updated |
...
**✅ v1 shipped 2026-08-18 (Phase 15, Tier 3).** New hero row (`AIActionCenter.tsx` span-3 + `PipelineFunnel.tsx` span-1) replaces the old 4 flat stat tiles entirely. Action Center is deterministic (`lib/dashboardInsights.ts`), NOT a live AI call per view — a dashboard loaded on every visit is the wrong place for per-load LLM cost/latency, matching this app's zero-marginal-cost philosophy already used for `lib/outcomeInsights.ts`/`lib/bait-and-switch` etc.; the "AI" framing describes the style of what's surfaced, not a runtime model call. Pipeline Funnel is real Draft→Applied→Interviewing→Offered counts, each segment linking to `/missions?stage=X`. **Corrected 2026-08-23 (Phase 22)**: "Draft" was quietly counting every found-but-never-reviewed job as pipeline backlog — split into a new pre-pipeline `inbox` status (excluded from this funnel entirely) and `shortlisted` (the funnel's real first segment now), see RESUME.md's Phase 22 entry. — `MissionsView.tsx` now reads that param to pre-select the matching List filter, honoring the research's "no dead ends" rule with real navigation rather than a second parallel drill-down UI. `ActivityHeatmap.tsx` (hand-built CSS grid, no new dependency) replaces both day-of-week bar/area charts (`CompanyResearchChart`/`JobsOverTimeChart`, both deleted — confirmed unused elsewhere first). `UpcomingInterviews.tsx` only renders when a real interviewing-stage job exists, per spec. `RejectionRadar.tsx` is reuse-only — surfaces jobs that already have a real `rejection_diagnosis`, never generates one. `MatchDistributionChart`/`RecentActivity` kept as-is, just repositioned into the new grid. **Deliberately not built**: inline hover-popup of the 10-dimension breakdown on Recent Activity rows, and Career Record "Weekly Wins" (both from the original research's wider wishlist, not in its own "concrete layout" 3-row spec). Live-verified: the funnel's "Interviewing" segment correctly navigated to a pre-filtered Missions List showing exactly the one real interviewing job; the heatmap's hover tooltip correctly showed "77 on 2026-08-13" matching real seeded data. `tsc --noEmit`/`eslint .` (whole project) clean.
...
| **Google Calendar interview detection** | ✅ shipped 2026-08-20 | Real Google Cloud OAuth app (`sortie-g1`/`javascript-nextjs`), user-created — `calendar.events.readonly` scope only (lighter Sensitive-tier verification, not Gmail's Restricted-tier/CASA-audit requirement). `lib/googleCalendar.ts` (dependency-free REST, no `googleapis` SDK), `actions/googleCalendar.ts`, OAuth start/callback routes, Settings → Google Calendar tab. Fetches the next 14 days of events, naive-matches event title against tracked (`is_hidden=false`) jobs' company names — genuinely sufficient at this app's pipeline size, no NLP needed. New `profiles.google_calendar_refresh_token`/`google_calendar_connected_at` columns. Still in Google's "Testing" status (test users only) until the account owner submits for verification. **Not live-tested end to end** — needs a real connected Google account, which only the user has. |
| **Outlook interview detection** | ✅ shipped 2026-08-20 | Real Azure AD app registration ("Sortie Integration," multitenant + personal accounts), `Calendars.Read` delegated scope only — mirrors the Google Calendar feature's shape exactly (`lib/outlookCalendar.ts`, `actions/outlookCalendar.ts`, Settings → Outlook Calendar tab). No CASA-equivalent paid audit needed on Microsoft's side for this scope. **Not live-tested end to end** — needs a real connected Microsoft account. |
| **Notion export** (two-way: push tracker + import leads) | ✅ shipped 2026-08-20 (Phase 16) | Personal internal-integration-token model (`notion.so/my-integrations`), not a public OAuth app — no external developer-console registration needed. `lib/notion.ts` (raw REST, no SDK dep) + `actions/notion.ts` + Settings → Notion tab. Push mirrors the Missions scope exactly (`is_hidden = false`, same as `app/missions/page.tsx`) into a user-owned Notion database, auto-provisioning the properties it needs (Company/Location/Status/Match score/URL) on connect. Read-back imports any page with a URL the user added directly in Notion as a new lead via the existing `createExternalJob`/Jina-reader path, reconciled via a new `jobs.notion_page_id` column (no dependency on custom Notion-side properties). New `profiles.notion_token/notion_database_id/notion_database_name/notion_last_synced_at` columns, added via plain `db query` (not `auth.users`-referencing DDL, so no migrations-tracking workaround needed). **Verification tier**: clean `tsc`/`eslint`/production build only — not live-tested end-to-end, since that needs a real Notion account/token which only the user has. |
| Obsidian export | ✅ shipped 2026-08-19 (Phase 19) | Markdown-only, one-way (`/api/career/export?format=markdown`) — no live sync exists since Obsidian has no public write API for a deployed web app |
...
| Global search | ✅ shipped 2026-08-28 (Phase 29/30) | Real full-text search across title/company/location/job-type/tags/personal-notes/description/requirements/responsibilities/nice-to-have/benefits/private reflection — a trigger-maintained `search_vector` tsvector column + GIN index + `search_jobs()` Postgres function (ranked via `ts_rank`, snippets via `ts_headline`), migration `20260828180000_add-full-text-job-search.sql`. `actions/jobs.ts`'s `searchJobsFullText()`, a dedicated `/search` page (`components/search/GlobalSearchView.tsx`), a persistent site-wide search bar rendered below the Navbar on every authenticated page (`components/layout/GlobalSearchBar.tsx`), and a "Search all jobs for…" cross-link from Cmd+K's own quickSearchJobs results. Live-verified against real DB data (real ranked results, real highlighted snippets, both via a standalone authenticated script and a real browser page load). |
...
| Bulk actions (multi-select archive/tag) | ✅ | **Marker corrected 2026-08-27 (Phase 28)**: `InboxTable.tsx`/Missions List had bulk Shortlist/Archive/Tag since Phase 22; Kanban board and Saved Jobs both gained the same select-mode/bulk-bar UI this session (`KanbanCard.tsx`'s drag handle swaps for a checkbox in select mode; `SavedJobsTabs.tsx` reuses `JobResultCard`'s existing `selectable` props) |
| Profile completeness meter | ✅ | **Marker corrected 2026-08-28 (Phase 28)**: already shipped — `lib/profile-utils.ts`'s `calculateCompletion()` + `ProfileAttentionBanner.tsx` (circular % ring + missing-field chips), rendered on `/dashboard` whenever completion < 100% |
...
| **Homepage static rendering / cold-start fix** | 🆕 real, diagnosed, not built — direct user request 2026-08-30 | `app/page.tsx` calls `getCurrentUser()` purely to pick a Navbar variant, which forces the entire route dynamic — confirmed live: `Cache-Control: no-store`/`X-Vercel-Cache: MISS` on every request, real cold-start hit ~2.7s, warm requests still 300-600ms with zero caching. `Navbar.tsx` is already a client component, so the real fix is: static homepage (assume logged-out by default) + a client-side auth check on mount to swap in the authenticated variant. Deliberately not built yet — it's the first browser-side InsForge client this codebase would need, and touches Navbar (every page), so it's parked pending explicit go-ahead. **Also still needed, not yet measured**: real *production* load times for the authenticated pages (Dashboard, Find Jobs, job detail) — only messy local dev-mode numbers exist so far, known to be inflated by on-demand compilation, not trustworthy for a real "is this actually slow" call. |
...
| Installable PWA + mobile responsive polish | 🟡 mostly shipped | **Marker corrected 2026-08-28 (Phase 30)** — installability was already substantially built before this session (manifest.json, real icons, global `ServiceWorkerRegisterLoader`) despite the stale "🆕 Brainstorm" marker; this session added the missing `appleWebApp` metadata for proper iOS "Add to Home Screen" behavior. Mobile-responsive polish: audited Dashboard/Missions/Career/Interview/Settings/Resume/Profile/Search at a real 375px viewport (`getBoundingClientRect()`-based overflow detection, not visual guessing) — found and fixed one real bug (Missions' `loading.tsx` skeleton had a fixed `w-96` subtitle placeholder overflowing at mobile width; same defensive fix applied to Career's `w-80` one, which wasn't yet overflowing but was the same fragile pattern). Every other page checked came back clean. **Not yet audited**: find-jobs, job-detail, admin surfaces, and a real touch-target/tap-size pass (this session only checked for horizontal overflow, not touch ergonomics). |
...
| Programmatic SEO pages built on Interview Prep question banks | ✅ shipped 2026-08-30 (Phase 33) | `/interview-questions` hub + `/interview-questions/[slug]`, real rows only from `interview_question_banks` (`lib/interviewSeo.ts`), `FAQPage` JSON-LD. Wasn't in `sitemap.ts` until Phase 34 caught and fixed it — see below. |
| Programmatic SEO pages built on real salary ranges | ✅ shipped 2026-08-30 (Phase 34) | `/salary-insights` hub + `/salary-insights/[slug]`, real employer-posted pay ranges parsed from `jobs.salary` (`lib/salaryInsightsSeo.ts`), 11 real (role, location) buckets at launch. Company-research dossiers were tried first and rejected after live verification found per-candidate résumé leakage and unreliable employer attribution — see progress-tracker.md's Phase 34 entry. Both this and the interview-questions pages above are now correctly listed in `sitemap.ts` (a real gap — interview-questions had no sitemap entry and no nav link since it shipped). |
| Career News section (cross-industry, real RSS + AI synthesis) | ✅ shipped 2026-08-30 (Phase 35) | Direct user request, iterated through 3 `agy` research passes after user pushback (rejected a tech-only category lineup, wanted a real browsable "Samsung News"-style destination, not just a buried widget). `/news` (public, category tabs: Hiring & Layoffs, AI & Future of Work) + `CareerRadar.tsx` (personalized dashboard widget, same `news_items` table filtered to the user's saved companies). `lib/newsIngestion.ts`, `lib/inngest/functions.ts`'s `syncNewsItemsAsync` (6h cron). $0 added infra cost — free RSS + Jina Reader + Gemini fast-tier, no paid news API. See progress-tracker.md's Phase 35 entry for full detail. |
...
| Recon (free) / Command ($15/mo) tiers, admin-managed & DB-driven | ✅ shipped 2026-08-21 | Direct user request — see RESUME.md's newest entry. `subscription_plans` (add/edit/remove tiers live from `/admin/billing`, no redeploy) + `user_subscriptions` + `api_usage_metrics` tables, `lib/subscription.ts` circuit breakers gate Apify/insider-connections, Browserbase/company-research, job evaluations, and the LLM router by plan. Per-user overrides (`profiles.feature_flags`) let admins comp one feature to one user regardless of plan. Admin/owner accounts are a true unmetered bypass, not just Command's cap. |
| Real Stripe billing (test-mode) — checkout, portal, subscription webhook fulfillment | ✅ shipped 2026-08-21 | Direct user request — real test-mode purchase completed end to end. Razorpay stays parked (real unresolved key errors). |
| Vanguard — $149 one-time "lifetime deal," 250-seat scarcity cap | ✅ shipped 2026-08-23 (Phase 22) | Direct user request. `subscription_plans.max_seats`/`seats_claimed`, race-safe `claim_plan_seat()` at Stripe fulfillment, one-time Checkout (mode=payment), scarcity progress bar + Sold Out UI, daily cron rolls the usage period forward. See RESUME.md/progress-tracker.md's Phase 22 entries. |
...
## S. Public Marketing Homepage & Logged-Out Experience — ✅ v1 shipped 2026-08-20 (Phase 20)

**v1 built and deployed** — full detail in `progress-tracker.md`'s Phase 20 entry. Fast map: `Navbar.tsx` split logged-out/authenticated, new `/methodology` page, `Hero.tsx` rebuilt (mission-console dark chrome + real evaluator preview), new `BentoFeatures.tsx` (replaces `Features.tsx`), new `AntiAutoApply.tsx`, new `TheLifecycle.tsx` (replaces `HowItWorks.tsx`), `SuccessStory.tsx` rebuilt (a real fabricated-testimonial honesty bug was found and fixed — see progress-tracker.md), `CTASection.tsx` reworked into a transparent pricing anchor. Old `landing-*` CSS confirmed unused and removed. **Not yet done, real fast-follows if picked up again**: the "Features" nav link is a same-page anchor, not the dedicated mega-menu the original research recommended; no dedicated `/pricing` page (the anchor section covers it for now); Core Web Vitals/Lighthouse pass not run; no analytics/conversion tracking added to the new CTAs specifically.

**Original research below, superseded by the above where they conflict** — kept for the original reasoning. **Status corrected 2026-08-30**: the mega-menu nav was already shipped in an earlier fast-follow (`Navbar.tsx`'s `featuresOpen` state, grouped by Discovery/Application/Interviews per this research) — this file's own "not yet built" note here was stale. Programmatic SEO pages (real interview questions by company/role, off the `interview_question_banks` table) shipped 2026-08-30 — `/interview-questions` (hub) + `/interview-questions/[slug]` (per-entry, `FAQPage` structured data), `lib/interviewSeo.ts`. The `/methodology` page already covers the "Why Auto-Apply is Broken" GEO recommendation too — also already built, not a gap.

**Direct user request, Phase 19**: "the biggest thing still need to log for the future build is how we can build brand new home page with all the features describing and also navigation for non sign up or new users with marketing in mind... spectacular gui." Researched via `agy -p` (needed `--dangerously-skip-permissions` to read local context files non-interactively — the first attempt without it failed outright with a permission error before producing any research; the flag itself then hit this session's own auto-mode classifier and needed explicit user confirmation before retrying, same as a production deploy). Grounded in this app's real shipped feature set (`build-plan.md`/`RESUME.md`/`progress-tracker.md`), not invented capabilities. **Research only — nothing below is built.** The current homepage (`app/page.tsx`, `components/homepage/*`) is a fairly generic Hero/HowItWorks/Features/SuccessStory/CTA layout; this reconsiders it from scratch with conversion as the primary goal.

**1. Information architecture** — reframe from generic "Hero/Features/CTA" to an outcome-driven "Product as Marketing" flow:
1. **"Mission Console" hero, outcome-oriented, not category-oriented** — e.g. "Your Explainable Career Agent. Land interviews, not auto-rejections," with an interactive dark-mode UI preview of the real 10-Dimension Evaluator or Insider Connections panel (not a static screenshot), primary CTA "Start for free" in `--color-accent`.
2. **Social proof / momentum, even pre-monetization** — if user counts aren't ready to publish, lead with technology proof instead ("Powered by Gemini, Claude, and OpenAI") rather than a fabricated number.
3. **Bento-grid feature showcase ("The Career OS")** — prevents a wall-of-text across the real 10+ feature set: 10-Dimension Evaluator (large card), ATS-Safe Tailoring (theme-switch visual), Insider Connections (3-bucket + email reveal visual), the Capture Extension ("Save → Grade → Track"), Model Agnosticism (the real Gemini/OpenAI/Claude router dropdown).
4. **A dedicated "anti-auto-apply" section** — "Quality over volume. You're not a spambot," explicitly the app's real, already-decided-and-repeatedly-reconfirmed differentiation (see the standing "Auto-apply stays explicitly ruled out" decision elsewhere in this file) turned into marketing copy instead of just an internal principle.
5. **The lifecycle section** — Kanban tracker blending into the real Interview Prep Engine (STAR stories, predicted questions).
6. **A transparent pricing/CTA anchor** — even pre-monetization, show today's real free tier plus a visibly "Coming Soon" Pro tier, for trust.

**2. Logged-out navigation** — separate the marketing journey from the app journey entirely: dark-ink-chrome nav with Features (mega-menu grouped by Discovery/Application/Interviews), a dedicated **Methodology** page explaining the 10-dimension rubric and the anti-auto-apply stance (flagged as genuinely trust-building, not filler), Pricing anchor, Log in (ghost) + Start for free (amber primary). A feature-specific landing page (e.g. `/features/resume-tailoring`) keeps the same nav but leads with that one feature, then links back to the holistic Career OS — never just the homepage reused under a different URL.

**3. Visual/GUI direction** — push the existing "mission console" language (dark ink chrome, amber signal, teal agent-radar) to a Linear/Vercel-grade execution: a subtle 1px grid/dot-matrix background behind hero/feature sections (signals "developer-grade tool"); rely on the existing surface ladder (`bg-background` → `bg-surface-tertiary`) + `border-border-light` instead of drop shadows, which read as muddy in dark mode; use `--color-agent` teal *exclusively* to highlight AI-generated content in product-preview mockups (same invariant as the rest of the app, extended into marketing copy); avoid scroll-jacking — micro-interactions only (amber border fade-in on card hover, a typed-out animation for "Agent read" explanations); keep the chrome permanently dark in both themes, content within it respects real light/dark tokens.

**4. Feature-description strategy** — group the 10+ real features by user journey instead of a flat list, to avoid fatigue: **Target** (discovery/evaluation: 10-Dimension Evaluator, Company/Leadership Intel, Capture Extension), **Attack** (tailoring/networking: ATS-Safe Tailoring, Insider Connections + email reveal), **Advance** (tracking/interviewing: Kanban Tracker, Interview Prep Engine), **Engine Room** (Model Router). Draft copy for each is in the full report (see below).

**5. SEO/GEO** — outcome-based title tags over the category-based current ones; a real **Methodology/Manifesto page** ("Why Auto-Apply is Broken") as the GEO play — AI answer engines (Perplexity/ChatGPT/Gemini) favor structured, opinionated stances when synthesizing "what's the best AI job search tool" answers, and this app already has a genuine, real, differentiated stance to publish, not a manufactured one; programmatic SEO pages off the Interview Prep engine (e.g. "Behavioral Interview Questions for Product Managers at Stripe") as a longer-term high-intent-traffic play; Core Web Vitals discipline — CSS grid lines + lightweight SVG motion, not video, so the "spectacular" GUI doesn't cost load speed.

**Full original report** (more copy drafts, more detail per section) saved outside the repo by `agy` at `C:/Users/ridha/.gemini/antigravity-cli/brain/d8039968-5daf-4a88-ab6a-a17cd1dd8aec/marketing-homepage-plan.md` — read that file directly if picking this up, don't only rely on this summary.

## L. B2B / white-label (the durable revenue destination)

**⏸ Explicitly deferred to post-launch, direct user decision (2026-08-27, Phase 28)** — "maybe" post-launch, not committed even then. Do not pick this section up unprompted; the whole section stays parked until the user raises it again.
