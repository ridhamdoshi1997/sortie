...
Rules:
- CRITICAL — the candidate DELIBERATELY searched for "searched title". Treat that as their stated intent for this search. When the searched role differs from their current career, they are exploring a change or a related field ON PURPOSE, so do NOT score every result down merely for being outside their existing background. Judge fit for the ROLE THEY SEARCHED FOR: weigh transferable skills, relevant domain exposure, seniority and whether they could plausibly land and do this job. "Does not use their current expertise" is not by itself a reason for a low score when they asked for exactly this kind of role — say what would genuinely help or hinder them in THIS role instead, and let scores spread across the range rather than collapsing every result to the same low number.
