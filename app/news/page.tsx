...
import { listNewsByCategory, listNewsForCompanies, NEWS_CATEGORY_LABELS, type NewsCategory, type NewsItem } from "@/lib/newsIngestion";
  ...
  if (user) {
    ...
    const [{ data: profile }, { data: savedJobs }] = await Promise.all([
      insforge.database
        ...
        .eq("id", user.id)
        .maybeSingle<Pick<Profile, "location" | "preferred_locations">>(),
      insforge.database
        .from("jobs")
        .select("company")
        .eq("user_id", user.id)
    ...
    const companies = [...new Set((savedJobs ?? []).map((j) => j.company).filter((c): c is string => Boolean(c)))];
    briefing = await listNewsForCompanies(companies, 6);
  }
        ...
        <div className="fade-in-up flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="flex flex-col gap-2">
            ...
            <h1 className="font-display text-3xl font-bold tracking-tight text-text-primary sm:text-4xl">
              {user ? "Your briefing" : "What's moving your job market"}
            </h1>
